...
Public Class GAD110_NT
    ...
    Private Function ExistData() As Boolean
        ...
        Dim dSet As DataSet = OpenDataSet("gad110_nt_datacheck", p)
    ...
    Private Function GetWorkChk(ByVal frTm As String, ByVal toTm As String, ByVal atnCd As String) As String
        ...
        Try
            ...
            Dim dSet As DataSet = OpenDataSet("gad110_nt_work_yn", p)
    ...
    Private Function GetStdTime() As Boolean
        Try
            ...
            Dim p As New OpenParameters
            p.Add("@bs_cd", bs_cd.Text)
            dSetBaseTime = OpenDataSet("gad110_nt_unit_time", p)
    ...
    Private Function GetDateBc(ByVal GetDt As String) As String
        ...
        Try
            ...
            p.Add("@bs_cd", bs_cd.Text)
            p.Add("@dt", GetDt)
            Dim dSet As DataSet = OpenDataSet("gad110_nt_dt_bc", p)
            If Not IsEmpty(dSet) Then
    ...
    Private 주야구분_주간고정 As String = "PP160200"
    ...
    ' Private 근무시간_조출 As String = "GA170110"

    'Private 근무시간_정상 As String = "GA170100"
    ''Private 근무시간_조출 As String = "GA170110"
    'Private 근무시간_연장1 As String = "GA170301"
    'Private 근무시간_연장2 As String = "GA170302"
    'Private 근무시간_연장3 As String = "GA170303"
    'Private 근무시간_연장4 As String = "GA170304"
    'Private 근무시간_연장5 As String = "GA170305"
    'Private 근무시간_연장6 As String = "GA170306"
    'Private 근무시간_연장7 As String = "GA170307"
    'Private 근무시간_연장8 As String = "GA170308"
    'Private 근무시간_연장9 As String = "GA170309"
    'Private 근무시간_연장10 As String = "GA170310"
    'Private 근무시간_연장11 As String = "GA170311"
            ...
            Try
                ...
                If stdTmRows.GetLength(0) > 0 Then
                    ...
                    If frTm <> "" And toTm <> "" And chkLate = "Y" Then
                        ...
                        If std_toTm > toTm And AtnCd <> 조출 Then
                            off_hr = HourDiff(toTm, std_toTm) + 0.49
                            off_hr = off_hr - CalLossTm(toTm, std_toTm, DnBc, DtBc, WorkBc) / 60.0
                            off_hr = DFix((off_hr) / 0.5) * 0.5
                        End If
                    End If
                End If
                ...
                If baseTmRows.GetLength(0) > 0 Then
                    ...
                    For gRow = 0 To baseTmRows.GetLength(0) - 1
                        ...
                        If AtnCd = 조출 And DnBc = 주야구분_야간 And (TmBc = 근무시간_연장6 Or TmBc = 근무시간_연장9) Then
                            ' Dim std_base As Decimal '조출후 기본 8시간 
                            'Dim Std_before As Decimal '조출후 야간 연장 시간(기본시간 이후 ~32:30분 )


                            '[조출계산 - 공제시간]
                            ' before_hr = CacuTm(frTm, toTm, frTm, toTm, set_frTm, set_toTm, 0, DnBc, DtBc)
                            ' before_hr = CacuTm(frTm, toTm, set_frTm, set_toTm, 2, DnBc, DtBc, WorkBc, WorkYn)
                            before_hr = CacuTm(frTm, set_frTm, frTm, set_frTm, 0, DnBc, DtBc, WorkBc, WorkYn)
                        ...
                        ElseIf TmBc = 근무시간_연장3 Then
                            ...
                            Else
                                ...
                                PutTime("std_hr", iRow, 0) '휴일정상
                            End If
                        ElseIf TmBc = 근무시간_연장6 Then
                            If toTm < set_frTm Then   '퇴근시간이 정상시작시간 이전 이면 정상근무 아님
                                std_hr = 0
                            Else
                                Dim frTm2 As String, toTm2 As String
                                ...
                                ''/*2012-04-04 조출이면서 정상시간 8시간 안될경우(조출이고 야간 일경우만 해당)***************************************************************************/
                                ''조출이면서 정상시간 8시간 이하일 경우 조출시간을 정상시간으로 계산하고 나머지가 연장으로
                                'If std_hr < 8 And before_hr > 0 Then
                                '    Dim Add_std As Decimal
                                '    Add_std = before_hr - (IIf(before_hr > (8 - std_hr), 8 - std_hr, before_hr))
                                '    'before_hr = before_hr - (IIf(before_hr > (8 - std_hr), 8 - std_hr, before_hr))
                                '    t6Hr = std_hr + (8 - IIf(before_hr > std_hr, before_hr, std_hr))
                                '    before_hr = Add_std
                                'End If
                                ''/****************************************************************************/

                                PutTime("std_hr", iRow, 0) '평일야간정상
                            End If
                        ElseIf TmBc = 근무시간_연장9 Then
                            If toTm < set_frTm Then   '퇴근시간이 정상시작시간 이전 이면 정상근무 아님
                                std_hr = 0
                            Else
                                Dim frTm2 As String, toTm2 As String
                                ...
                                ' 조출이면서 8시간이 아닌경우에는 정상에 조출을 더해준다

                                '    If std_hr < 8 Then
                                '        'If before_hr - (8 - std_hr) >= 0 Then
                                '        std_hr = (8 - std_hr) + (before_hr - (before_hr - ((8 - std_hr))))
                                '        before_hr = before_hr - (8 - std_hr)
                                '        'End If
                                ...
                                '    Add_std = before_hr - (IIf(before_hr > (8 - std_hr), 8 - std_hr, before_hr))
                                '    'before_hr = before_hr - (IIf(before_hr > (8 - std_hr), 8 - std_hr, before_hr))
                                '    t9Hr = std_hr + (8 - IIf(before_hr > std_hr, before_hr, std_hr))
                                '    before_hr = Add_std
                                'End If
                                ''/****************************************************************************/


                            PutTime("std_hr", iRow, 0) '휴일야간정상
                            End If
                        Else


                            tHr = CacuTm(org_frTm, org_toTm, set_frTm, set_toTm, 1, DnBc, DtBc, WorkBc, WorkYn)
                ...
                Dim gapHr As Decimal = 0
                '조출이면
                If .Text("atn_cd") = "C3301" Then

                    '평일 이고 정상시간이 8 시간이하이면
                    If 0 < std_hr And std_hr < 8 Then
                        '연장시간이 있으면 연장시간-> 정상시간
                        If 0 < t1Hr Then
                            '8시간을 채우는데 필요한시간
                            gapHr = 8 - std_hr
                    ...
                    '휴일 이고 정상시간이 8 시간이하이면
                    If 0 < t3Hr And t3Hr < 8 Then
                        '연장시간이 있으면 연장시간-> 정상시간
                        If 0 < t4Hr Then
                            '8시간을 채우는데 필요한시간
                            gapHr = 8 - t3Hr
                    ...
                    '야간 이고 정상시간이 8 시간이하이면
                    If 0 < t6Hr And t6Hr < 8 Then
                        '연장시간이 있으면 연장시간-> 정상시간
                        If 0 < t7Hr Then
                            '8시간을 채우는데 필요한시간
                            gapHr = 8 - t6Hr
                    ...
                    '휴일야간 이고 정상시간이 8 시간이하이면
                    If 0 < t9Hr And t9Hr < 8 Then
                        '연장시간이 있으면 연장시간-> 정상시간
                        If 0 < t10Hr Then
                            '8시간을 채우는데 필요한시간
                            gapHr = 8 - t9Hr
                ...
                '평일야간이면
                If .Text("dt_bc") = "GA120100" And .Text("dn_bc") = "PP160150" Then
                    '8시간이 안되면
                    If 0 < t6Hr And t6Hr < 8 Then
                        If .Text("fr_tm") > "20:00" Then
                            '지각
                            late_hr = 8 - t6Hr
                        Else
                            '조퇴
                ...
                '   PutTime("t12_hr", iRow, before_hr) '조출시간

                ' PutTime("before_hr", iRow, before_hr) '조출시간
