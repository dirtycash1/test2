...
Public Class PMB100
    ...
    Private Sub Tree_Open()
        Try
            ...
            'Command 생성
            dSet = Me.OpenDataSet("pmb100_Tree", P)
    ...
    Private Sub work_btn_Click(sender As System.Object, e As System.EventArgs) Handles work_btn.Click
        ...
        If MessageYesNo("가져오기 하시겠습니까?") = MsgBoxResult.No Then
            Exit Sub
        End If

        If temp_cd.Text = "" Then
            Return
        Else
            'Dim P As New OpenParameters
            'P.Add("@temp_cd", temp_cd.Text)
            dSet = Me.OpenDataSet("pmb100_work") ', P)
            If Not IsEmpty(dSet) Then
                For Each dRow As Data.DataRow In dSet.Tables(0).Rows
                    g20.AddNewRow()
                    g20.Text("up_no", g20.RowIndex) = dRow("up_no")
                    g20.Text("step_no", g20.RowIndex) = dRow("step_no")
                    g20.Text("step_nm", g20.RowIndex) = dRow("step_nm")
                    g20.Text("step_bc", g20.RowIndex) = dRow("step_bc")
                Next
            End If
        End If
