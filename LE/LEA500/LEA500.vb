...
Public Class LEA500
    Private Sub LEA500_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        With g20
            ...
            For i As Integer = 0 To .RowCount - 1
                If .ToBool("chk", i) Then
                    ...
                    If .Text("cust2_cd", i) = "" Then
                        MessageInfo("매입처는 필수입니다. [기준정보_품목별 매입처등록]에서 품목별 매입처를 등록 바랍니다")
                        Exit Sub
                    End If
                    ...
                    ' 동일 거래처  체크
                    If custCd = "" Then
                        custCd = .Text("cust2_cd", i)
                    End If

                    If custCd <> .Text("cust2_cd", i) Then
    ...
    Private Function _Work() As Boolean
        ...
        If g20.Text("chk") <> "1" Then
            MessageInfo("선택된 자료가 없습니다.")
            Exit Function
        End If
        ...
        Dim dSet As DataSet = Me.OpenDataSet("lea500_save_data")
        ...
        If Me.Save Then
            Dim dSet2 As DataSet = Me.OpenDataSet("lea500_work")
    ...
    Private Sub Sum_All()
        Dim iRow As Integer, amt As Double, vat As Double, cnt As Integer, qty As Double
        Dim lastRow As Integer = -1, vat2 As Double = 0
        ...
        With g20
            For iRow = 0 To .RowCount - 1
                ...
                If .ToBool("chk", iRow) Then
                    cnt += 1
                    'qty += .ToDec("in_qty", iRow)  기준단위수량(환산된 재고수량)
                    qty += .ToDec("um_qty", iRow)   ' 입고수량
            ...
            If ToDec(amt) = 0 Or vat_rt.ToDec = 0 Then
                vat2 = 0
            Else
                vat2 = Math.Truncate(amt / vat_rt.ToDec)
            End If
            ...
            If vat <> vat2 Then
                If -1 < lastRow Then
                    m_StopEvent = True
                    .Text("set_vat", lastRow) = .ToDec("set_vat", lastRow) + vat2 - vat
                    m_StopEvent = False
                End If
            End If
    ...
    Private Sub g20_CellValueChanged(ByVal sender As Object, ByVal ColumnName As String, ByVal RowIndex As Integer, ByRef Value As Object) Handles g20.CellValueChanged
        ...
        If ColumnName = "set_amt" Or ColumnName = "vat_rt" Then
            g20.Text("set_vat") = Int(g20.ToDec("set_amt") * g20.ToDec("vat_rt") * 0.01)
        End If
    ...
    Private Sub g20_CellValueChanging(ByVal sender As Object, ByVal ColumnName As String, ByVal RowIndex As Integer, ByRef Value As Object) Handles g20.CellValueChanging
        ...
        With g20
            ...
            '170303smk
            If g20.Text("doc_no") = "" Then
                doc_no.Text = ""
            Else
                doc_no.Text = g20.Text("doc_no")
            End If


            If .Text("doc_no") <> "" Then

                If ColumnName = "doc_yn" Then   '161101smk 조건 
                Else
                    MessageInfo("이미 전표가 발행되었습니다.")
                    .UpdateRow()
                    Exit Sub
            ...
            If ColumnName = "chk" Then
                If Value = "1" Then
                    Dim custCd As String
                    custCd = .Text("cust2_cd")

                    '  동일 거래처  체크
                    For iRow = 0 To .RowCount - 1
                        If .ToBool("chk", iRow) And custCd <> .Text("cust2_cd", iRow) Then
                            MessageInfo("동일한 매입처만 전표처리 가능 합니다")
                            .UpdateRow()
                            '.Text("chk") = ""
                            Exit Sub
                    ...
                    vat_rt.Text = .Text("vat_rt")

                    'Me.Sum_All()

                    '입고단가를 정산단가로 'park test
                    g20.Text("set_up") = g20.Text("in_up")
                Else
                    g20.Text("set_up") = ""
            ...
            Try
                ...
                For R = 0 To .RowCount - 1
                    ...
                    If (.ToBool("chk", R) Or .ToBool("ok_yn", R)) And .Text("doc_no", R) = "" Then
                        ...
                        p.Add("@cust_cd", .Text("cust2_cd", R))
                        p.Add("@cury_bc", .Text("cury_cd", R))
                        p.Add("@po_fac", .Text("fac_cd", R))
                        p.Add("@in_dt", .Text("in_dt", R))

                        Dim Dset As DataSet = OpenDataSet("lea500_cost", p)
    ...
    Private Sub chk_all_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chk_all.CheckedChanged
        ...
        If chk_all.Checked Then
            ...
            With g20
                For iRow = 0 To .RowCount - 1
                    If cust_cd.Text = .Text("cust2_cd", iRow) Then
    ...
    Private Sub g20_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles g20.DoubleClick
        ...
        If g20.FieldName(g20.ColumnIndex) = "cust2_nm" Then
            cust_cd.Text = g20.Text("cust2_cd")
            Me.New_Form()
        ElseIf g20.FieldName(g20.ColumnIndex) = "doc_no" Then
            doc_no.Text = g20.Text("doc_no")
        End If
    End Sub
    ...
    '전표를 강제로 미발행하겠다는 버튼 (매입전표가 아닌 재산같은거)
    Private Sub btn_doc_yn_Click(sender As System.Object, e As System.EventArgs) Handles btn_doc_yn.Click
        Dim p As New OpenParameters
        Dim R As Integer
        ...
        With g20
            ...
            Try
                ...
                For R = 0 To .RowCount - 1
                    If .Text("doc_yn", R) = "1" Or .Text("doc_yn", R) = "0" Then
                        p.Add("@in_no", .Text("in_no", R))
                        p.Add("@in_sq", .Text("in_sq", R))
                        p.Add("@doc_yn", .Text("doc_yn", R))
                        Me.Open("lea500_doc_yn", p)
                    End If
    ...
    Private Sub g20_Load(sender As Object, e As EventArgs) Handles g20.Load
