...
Public Class WI_PR1124
    ...
    Private Function Saves() As Boolean
        ...
        Else
            '170403smk
            If temp_gong_cd.Text = "80" Then
                For i As Integer = 2 To 2
                    'MsgBox(g_body5.ToDec("FINAL_RESULT", 2))
                    If spec_val.ToDec <> g_body5.ToDec("FINAL_RESULT", 2) Then
                        If MessageYesNo("▶POWER확인◀ 입고파워와 최종파워가 다릅니다! 그래도 저장 하시겠습니까?") = MsgBoxResult.No Then
                            Exit Function
                        Else   '그래도저장하겠다
                            Exit For
                        End If
    ...
    Private Sub g_list2_DoubleClick(sender As Object, e As System.EventArgs) Handles g_list2.DoubleClick
        ...
        '20161220 부자재 창고 재추가
        If g_list2.Text("FAC_CD", g_list2.RowIndex) = "02" Then
        ...
        '20160225smk 멸균NO 제어 170118smk
        If temp_gong_cd.Text = "70" Or temp_gong_cd.Text = "80" Then   '규격:멸균no수정x
            mate_no.ReadOnly = True
        Else
            mate_no.ReadOnly = False
        End If


        'If g_list2.Text("FAC_CD", g_list2.RowIndex) = "02" Then   '2공장 
        '    If temp_gong_cd.Text = "70" Or temp_gong_cd.Text = "80" Then   '규격:멸균no수정x
        '        mate_no.ReadOnly = True
        '    Else
        '        mate_no.ReadOnly = False
        ...
        'Else
        '    If temp_gong_cd.Text = "80" Then   '규격:멸균no수정x
        '        mate_no.ReadOnly = True
        '    Else
        '        mate_no.ReadOnly = False
        '    End If
    ...
    Private Sub g_list_DoubleClick(sender As Object, e As System.EventArgs) Handles g_list.DoubleClick
        ...
        '20160310smk 지시파워=실적파워 일치여부 확인 
        Dim dSet30 As Data.DataSet = Me.OpenDataSet("wi_pr1124_spec30", p)
        Dim dRow30 As DataRow, dRows30 As DataRowCollection
        dRows30 = dSet30.Tables(0).Rows

        If Not IsEmpty(dSet30) Then
            For Each dRow30 In dRows30
                spec30_gap.Text = ToStr(dRow30("GAP"))
                spec30_yn.Text = ToStr(dRow30("YN"))

                If ToStr(dRow30("YN")) = "N" Then
                    spec30_gap.TitleBackColor = Color.Red
                Else
                    spec30_gap.TitleBackColor = Color.Transparent
                End If

            Next
        Else
            spec30_gap.Text = "비교불가(문자)"
            spec30_gap.TitleBackColor = Color.LightSkyBlue
        End If
        ...
        'mate_no.ReadOnly = False
        ' 20160225smk 멸균NO 제어 170118smk
        If gong_cd.Text = "70" Or gong_cd.Text = "80" Then   '규격:멸균no수정x
            mate_no.ReadOnly = True
        Else
            mate_no.ReadOnly = False
        End If
    ...
    Private Sub setJobNO()
        ...
        If Not IsEmpty(dSet) Then
            ...
            For Each dRow In dRows
                pr_no.Text = ToStr(dRow("pr_no")) '20151027 양홍석
                stts.Text = ToStr(dRow("stts")) '20151027 양홍석
                job_no.Text = ToStr(dRow("job_no")) '20151027 양홍석
                ...
                mate_no.Text = ToStr(dRow("MATE_NO")) '20151027 양홍석
                u_whcd_yn.Text = ToStr(dRow("u_whcd_yn")) '20151027 양홍석
                lot_no2.Text = ToStr(dRow("LOT_NO2")) '20151027 양홍석
            Next
        Else
            Me.Clear()
    ...
    Private Sub spec_print_Click(sender As System.Object, e As System.EventArgs) Handles spec_print.Click
        ...
        If Mid(lot_no.Text, 1, 1) <> "B" And Mid(lot_no.Text, 1, 1) <> "b" Then
            ...
            If lot_no.Text <> "" And stts.Text = "C" And '170123smk 조선미요청
                gong_cd.Text = "80" Then
                '   
                System7.ReportView.LoadView("WI_PR1124", "", "WI_PR1124_print", p, , False)
            Else
                MessageInfo("규격 검사 확인상태만 출력 할수 있습니다. 또는 체크시트 번호가 없습니다.")
            End If
        Else
            ' MessageInfo("1")
            p.Add("@lot_no", lot_no.Text)
            ...
            Else
                MessageInfo("규격 검사확인상태만 출력 할수 있습니다. 또는 체크시트 번호가 없습니다.")
            End If
        End If
    ...
    Private Sub btn_start_Click(sender As Object, e As System.EventArgs) Handles btn_start.Click  '작업시작버튼 클릭->멸균no변경
        'from_time.Text = setTime()

        ''smk작업시작일자 변경될때, 멸균NO변경
        'p.Clear()
        'p.Add("@from_dt", from_dt.Text)
        'Dim dSet As Data.DataSet = Me.OpenDataSet("wi_pr1124_mateno", p)

        'If Not IsEmpty(dSet) Then
        '    Dim dRow As DataRow, dRows As DataRowCollection
        '    dRows = dSet.Tables(0).Rows
        '    For Each dRow In dRows
        '        mate_no.Text = ToStr(dRow("LOT_NO"))
        '    Next
        'Else
        'End If
    ...
    Private Sub pr_dt_TextChanged(sender As Object, e As System.EventArgs) Handles pr_dt.TextChanged

        'smk160225 생산일자 변경되면, 멸균NO도 변경

        If temp_gong_cd.Text = "70" Or temp_gong_cd.Text = "80" Then

        Else
            Dim p2 As New OpenParameters
            '규격 아닐때 
            p2.Clear()
            p2.Add("@from_dt", pr_dt.Text)
            Dim dSet As Data.DataSet = Me.OpenDataSet("wi_pr1124_mateno", p2)
    ...
    Private Sub new_gdcd_TextChanged(sender As Object, oldValue As String) Handles new_gdcd.TextChanged
        '20160225smk 멸균NO 제어
        'If temp_gong_cd.Text = "70" Or temp_gong_cd.Text = "80" Then   '규격:멸균no수정x
        '    mate_no.ReadOnly = True
        'Else
