...
Public Class WI_PR1129
    Dim p As New OpenParameters
    Dim g_jobno As String
    ...
    Private Function Check_Stts() As String
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("WI_PR1129_stts", p)
    ...
    Private Sub Max_NO()
        ' p/o 번호 채번
        pr_no.CodeNo = "WI_PR1126"        'Function dbo.[fnCodeNo]안에 LEB100에 대한 코딩을 해야 한다
        pr_no.CodeDateField = pr_dt

        If lot_no.Text = "" Then lot_no.Text = Max_LOTNO()
    End Sub

    Private Function Max_LOTNO() As String
        p.Clear()
        p.Add("@GD_NM", pr_dt.Text)

        Dim dSet As Data.DataSet = Me.OpenDataSet("WI_PR1129_maxLOTNO", p)
        Dim stts As String = ""
    ...
    Private Function ConFrim() As Boolean
        ...
        Else
            ...
            'Me.Open("WI_PR1129_confirm")

            p.Clear()
            p.Add("@pr_no", pr_no.Text)
            p.Add("@stts", "C")

            Me.Open("WI_PR1129_confirm", p)
    ...
    Private Function Check_deconfirm() As String
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("WI_PR1129_deconfirm", p)
    ...
    Private Sub Clear()
        ...
        Me.Open("WI_PR1129_head", p)
        Me.Open("WI_PR1129_body", p)
        Me.Open("WI_PR1129_body1", p)
        Me.Open("WI_PR1129_body2", p)
        Me.Open("WI_PR1129_body3", p)
        Me.Open("WI_PR1129_body4", p)
        Me.Open("WI_PR1129_body5", p)

        p.Clear()
        p.Add("@LOT_NO", "XXX")
        Me.Open("WI_PR1129_list3", p)
    ...
    Private Sub totQty2()
        ...
        With g_body
            .UpdateRow()
            For i As Integer = 0 To .RowCount - 1
                sum_amt = sum_amt + .ToDec("TUIP_QTY", i)
            Next
        End With
        pr_qty.Text = sum_amt.ToString
        tot_qty.Text = pr_qty.ToDec + ng_qty.ToDec + sample_qty.ToDec
    ...
    Private Sub g_list_DoubleClick(sender As Object, e As System.EventArgs) Handles g_list.DoubleClick
        ...
        p.Clear()
        p.Add("@LOT_NO", g_list.Text("JOB_NO", g_list.RowIndex))
        Me.Open("WI_PR1129_list3", p)
    ...
    Private Sub setJobNO()
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("WI_PR1129_jobno", p)
        ...
        dSet = Me.OpenDataSet("WI_PR1129_bom", p)
    ...
    Private Sub btn_contiue_Click(sender As Object, e As System.EventArgs) Handles btn_contiue.Click
         g_jobno = job_no.Text 'lot_no.Text
    ...
    Private Sub job_no_TextChanged(sender As Object, oldValue As String) Handles job_no.TextChanged
        setJobNO()
    End Sub
    ...
    Private Sub g_body2_CellValueChanged(sender As Object, ColumnName As String, RowIndex As Integer, ByRef Value As Object) Handles g_body2.CellValueChanged
        ' If g_body2.FieldName(g_body2.ColumnIndex + 1) <> "TUIP_QTY" Then Exit Sub
        'MsgBox(g_body1.FieldName(g_body1.ColumnIndex))
        totQty2()
        'If Mid(g_body2.Text("GD_CD", g_body2.RowIndex), 1, 1) = "I" Then
        '    pr_qty.Text = g_body2.Text("TUIP_QTY", RowIndex)
        'End If
    End Sub
    ...
    Private Sub g_body_CellValueChanged(sender As Object, ColumnName As String, RowIndex As Integer, ByRef Value As Object) Handles g_body.CellValueChanged
        If Mid(g_body.Text("GD_CD", g_body.RowIndex), 1, 1) = "I" Then
            pr_qty.Text = g_body.Text("TUIP_QTY", RowIndex)
            tot_qty.Text = pr_qty.ToDec + ng_qty.ToDec + sample_qty.ToDec
        End If
