...
Public Class WP_PR1241
    ...
    Private Sub mc_cdopen()
        Dim param As OpenParameters = New OpenParameters
        param.Add("@mc_cd", mc_cd.Text)
        Me.Open("WP_PR1241_mccd", param)
    End Sub
    Private Sub OpenGrid()
        Dim param As OpenParameters = New OpenParameters
        param.Add("@STD_DT", Fdt.Text)
        param.Add("@model_no", model_no.Text)
        param.Add("@inspec30", inspec30.Text)
        param.Add("@outspec30", outspec30.Text)
        param.Add("@gong_cd", gong_cd.Text)
        param.Add("@mc_cd", mc_cd.Text)
        Me.Open("WP_PR1241_G20", param)
    ...
    Private Sub OpenChart()
        Try
            ...
            'Dim dSet1 As DataSet = Me.OpenDataSet("WP_EIS1150_tab1_chart1", param)
            'Dim dSet2 As DataSet = Me.OpenDataSet("WP_EIS1150_tab2_chart1", param)

            'Me.Chart1(chart1_1, dSet1.Tables(0))
            'Me.Chart1(chart2_1, dSet2.Tables(0))

            'LoadedGridDataDrawChart2(g10, chart1_2, "일")
            LoadedGridDataDrawChart2(g20, chart2_2, "일")
            ' LoadedGridDataDrawChart2(g20, chart2_3, "일")
            isHap = True
    ...
    Private Sub SetColumnVisible(grid As Frame7.eGrid, Visible As Boolean)
        Dim to_dt = Convert.ToInt32(Fdt.Text.Substring(8, 2))
    ...
    Private Sub g20_Click(sender As System.Object, e As System.EventArgs) Handles g20.Click
        'ClickCreateChart2(g20, chart2_2, "일")
    End Sub
...
#Region "챠트2"
    ...
    Private Sub LoadedGridDataDrawChart2(grid As eGrid, ChartControl As DevExpress.XtraCharts.ChartControl, Day_Month_Unit As String, Optional MouseEvent As Boolean = False)
        ...
        If isHap = False Then
            For RowCount = 0 To grid.RowCount - 1
                ...
                ElseIf row(0) = "400" Then
                    chart2_3.Series.Add(CreateChart2Series(MakeDataTable(row, Day_Month_Unit), row, Day_Month_Unit))
                ElseIf row(0) = "810" Then
                    ChartControl.Series.Add(CreateChart2Series(MakeDataTable(row, Day_Month_Unit), row, Day_Month_Unit))
                ElseIf row(0) = "800" Then
                    chart2_3.Series.Add(CreateChart2Series(MakeDataTable(row, Day_Month_Unit), row, Day_Month_Unit))

                    'ElseIf row(0) = "600" Then
                    '    chart2_4.Series.Add(CreateChart2Series(MakeDataTable(row, Day_Month_Unit), row, Day_Month_Unit))
                    'ElseIf row(0) = "840" Then
                    '    chart2_4.Series.Add(CreateChart2Series(MakeDataTable(row, Day_Month_Unit), row, Day_Month_Unit))
    ...
    Private Function CreateChart2Series(table As DataTable, row As DataRow, Day_Month_Unit As String) As DevExpress.XtraCharts.Series
        ...
        Dim Series1 As New DevExpress.XtraCharts.Series(row(1), ViewType.Line)
        CType(Series1, System.ComponentModel.ISupportInitialize).BeginInit()
    ...
    Private Function MakeDataTable(row As DataRow, Day_Month_Unit As String) As DataTable
        ...
        Dim to_dt = Convert.ToInt32(Fdt.Text.Substring(8, 2))
        Dim to_mon = Convert.ToInt32(Fdt.Text.Substring(5, 2))

        If Day_Month_Unit = "일" Then
            For i = 1 To to_dt
                Dim dr As DataRow = table.NewRow()

                If ToDec(row(ColNm(i))) <> 0 Then
