...
Public Class SATR1124
    ...
    Private Sub Me_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load '첫오픈시, 행추가를 위함.
        ...
        p.Add("@emp_no", "")
        p.Add("@od_dt", "")
        Me.OpenDataSet("satr1124_g_multi", p)
        Me.Open()
    End Sub
    ...
    Private Sub btn_map_Click(sender As System.Object, e As System.EventArgs) Handles btn_map.Click
        ...
        With g_multi
            For i As Integer = 0 To g_multi.RowCount - 1
                p.Clear()
                p.Add("@fc_nm", g_multi.Text("FC_NM", i))   '거래처코드
                Dim dSet1 As Data.DataSet = Me.OpenDataSet("satr1124_map1", p)
                Dim dRow1 As DataRow, dRows1 As DataRowCollection
                dRows1 = dSet1.Tables(0).Rows

                p.Add("@sty_nm", g_multi.Text("STY_NM", i))   '상품코드
                Dim dSet2 As Data.DataSet = Me.OpenDataSet("satr1124_map2", p)
                ...
                Dim dSet3 As Data.DataSet = Me.OpenDataSet("satr1124_map3", p)
                Dim dRow3 As DataRow, dRows3 As DataRowCollection
                dRows3 = dSet3.Tables(0).Rows

                If IsEmpty(dSet3) Then
                    g_multi.Text("OD_PRI", i) = ""
                Else
                    For Each dRow3 In dRows3
                        g_multi.Text("OD_PRI", i) = ToDec(dRow3("PRICE"))
                        g_multi.Text("OD_AMT", i) = CLng(g_multi.ToDec("OD_QTY", i) * ToDec(dRow3("PRICE")))
                    Next
                End If
                ...
                '제품 할인율 맵핑
                p.Add("@gd_cd", g_multi.Text("GD_CD", i))   '제품코드
                Dim dSet4 As Data.DataSet = Me.OpenDataSet("satr1124_discount", p)
                Dim dRow4 As DataRow, dRows4 As DataRowCollection
                dRows4 = dSet4.Tables(0).Rows

                If IsEmpty(dSet4) Then
                    g_multi.Text("DISCOUNT_RT", i) = ""
                Else
                    For Each dRow4 In dRows4
                        g_multi.Text("DISCOUNT_RT", i) = ToDec(dRow4("RATE_RT"))
                        g_multi.Text("DISCOUNT_AMT", i) = CLng((g_multi.ToDec("OD_QTY", i) * g_multi.ToDec("OD_PRI", i)) * (ToDec(dRow4("RATE_RT")) / 100))
                    Next
                End If
    ...
    Private Function wf_odno() As Integer
        ...
        For i As Integer = 0 To g_multi.RowCount - 1
            p.Clear()
            p.Add("@fc_nm", g_multi.Text("FC_NM", i))
            p.Add("@od_dt", g_multi.Text("OD_DT", i))

            Dim dCnt As Data.DataSet = Me.OpenDataSet("satr1124_cnt", p)
            Dim dRow1 As DataRow, dRows1 As DataRowCollection
            ...
            Dim dSet As Data.DataSet = Me.OpenDataSet("satr1124_same_fc", p)
