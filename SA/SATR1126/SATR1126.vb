...
Public Class SATR1126
    ...
    Private Sub Me_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load '첫오픈시, 행추가를 위함.
        ...
        p.Add("@emp_no", "")
        p.Add("@od_dt", "")
        Me.OpenDataSet("satr1126_g_multi", p)
        Me.Open()
    End Sub
    ...
    Private Sub btn_map_Click(sender As System.Object, e As System.EventArgs) Handles btn_map.Click
        ...
        With g_multi
            For i As Integer = 0 To g_multi.RowCount - 1
                ...
                Dim dSet4 As Data.DataSet = Me.OpenDataSet("satr1126_map3", p)
                ...
                If g_multi.Text("OD_AMT", i) <> "" Then
                    ...
                    Dim dSet3 As Data.DataSet = Me.OpenDataSet("satr1126_discount", p)
                    ...
                    For Each dRow3 In dRows3
                        ...
                        '부가세/합계금액 추가SMK 160428
                        g_multi.Text("VAT", g_multi.RowIndex) = ToStr((g_multi.Text("OD_AMT", g_multi.RowIndex) - g_multi.Text("DISCOUNT_AMT", g_multi.RowIndex)) / 10)
                        g_multi.Text("SUM_AMT", g_multi.RowIndex) = ToStr(g_multi.Text("OD_AMT", g_multi.RowIndex) + ((g_multi.Text("OD_AMT", g_multi.RowIndex) - g_multi.Text("DISCOUNT_AMT", g_multi.RowIndex)) / 10))
                    Next
    ...
    Private Function wf_odno() As Integer
        ...
        For i As Integer = 0 To g_multi.RowCount - 1
            ...
            Dim dCnt As Data.DataSet = Me.OpenDataSet("satr1126_cnt", p)
            Dim dRow1 As DataRow, dRows1 As DataRowCollection
            ...
            Dim dSet As Data.DataSet = Me.OpenDataSet("satr1126_same_fc", p)
