...
Public Class WI_SA1120
    ...
    Private Function Check_Confirm_gu() As String
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1120_confirm_gu", p)
        Dim stts As String = ""
    ...
    Private Sub setPrice(hot_cd As String)
        ...
        For Each dRow In dRows
            ...
            If g_body.Text("OD_QTY", g_body.RowIndex) = "" Then
                If od_gu.Text = "CD05203" Or od_gu.Text = "CD05205" Or od_gu.Text = "CD05208" Or od_gu.Text = "CD05209" Then '위탁, 프로모션샘플, 샘플, 지급품
                    g_body.Text("OD_PRI", g_body.RowIndex) = 0
                    g_body.Text("DISCOUNT_RT", g_body.RowIndex) = 0
                Else
                    If ToStr(dRow("price")) = 0 Then
                        g_body.Text("OD_PRI", g_body.RowIndex) = ""
                        g_body.Text("AMT", g_body.RowIndex) = ""
                        g_body.Text("DISCOUNT_AMT", g_body.RowIndex) = ""
                        g_body.Text("DISCOUNT_RT", g_body.RowIndex) = ""
                        g_body.Text("OT_VAT", g_body.RowIndex) = ""
                        g_body.Text("OT_TOT", g_body.RowIndex) = ""
                    Else
            ...
            'If od_gu.Text = "CD05203" Or od_gu.Text = "CD05205" Or od_gu.Text = "CD05208" Or od_gu.Text = "CD05209" Then    '151019 smk
            '    g_body.Text("OD_PRI", g_body.RowIndex) = 0
            '    g_body.Text("DISCOUNT_RT", g_body.RowIndex) = 0
            'Else
            '    g_body.Text("OD_PRI", g_body.RowIndex) = ToStr(dRow("price"))
    ...
    Private Sub setPrice_jacd()
        ...
        'Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1120_price_jacd", p)
        'Dim price As Decimal = 0, rate_rt As Decimal = 0
    ...
    Private Function Saves() As Boolean
        ...
        If cs_gu.Text <> "CD0541" Then    '160323smk (백요청: 거래처구분 <> '정상' 체크)

            If MessageYesNo("정상 거래처가 아닙니다. 그래도 저장 하시겠습니까?") = MsgBoxResult.No Then
    ...
    Private Sub UNIT_QTY()  '낱개수량 구하기
        ...
        'Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1120_CLALEN", p)
        'Dim dRow As DataRow, dRows As DataRowCollection
        ...
        'Dim dSet2 As Data.DataSet = Me.OpenDataSet("wi_sa1120_IRIS", p)
    ...
    Private Sub popUP(hot_cd As String)
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1120_hot_cd", p)
    ...
    Private Sub spec(hot_cd As String)
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1120_spec", p)
        'Dim nm_cd As String = "", nm_nm As String = ""
        Dim dRow As DataRow, dRows As DataRowCollection
    ...
    Private Sub hot_spec(hot_cd As String)
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1120_hot_spec", p)
        'Dim nm_cd As String = "", nm_nm As String = ""
        Dim dRow As DataRow, dRows As DataRowCollection
    ...
    Private Sub g_body_CellValueChanged(sender As Object, ColumnName As String, RowIndex As Integer, ByRef Value As Object) Handles g_body.CellValueChanged
        ...
        Select Case ColumnName
            ...
            Case "GD_CD"
                ...
                'If g_body.Text("OD_QTY", g_body.RowIndex) = "" Then    '151019 SMK '161024 막음 
                '    g_body.Text("OD_PRI", g_body.RowIndex) = ""
                '    g_body.Text("AMT", g_body.RowIndex) = ""
                '    g_body.Text("DISCOUNT_AMT", g_body.RowIndex) = ""
                '    g_body.Text("OT_VAT", g_body.RowIndex) = ""
    ...
    Private Sub g_body_AfterMoveColumn(sender As Object, PrevColumnName As String, ColumnName As String) Handles g_body.AfterMoveColumn
        'If g_body.FieldName(g_body.ColumnIndex) = "SPEC10" The
    ...
    Private Sub totAmt()
        ...
        With g_body
            ...
            For i As Integer = 0 To .RowCount - 1
                ...
                Else  '그외 로우
                    'If .Text("OD_QTY", g_body.RowIndex) = "" Then   '151013 smk
                    '    .Text("AMT", g_body.RowIndex) = ""
                    '    .Text("DISCOUNT_AMT", g_body.RowIndex) = ""
                    '    .Text("OD_PRI", g_body.RowIndex) = ""
                    '    .Text("DISCOUNT_AMT", g_body.RowIndex) = ""
                    '    .Text("OT_VAT", g_body.RowIndex) = ""
                    '    .Text("OT_TOT", g_body.RowIndex) = ""
    ...
    Private Sub cs_cd_TextChanged(sender As Object, oldValue As String) Handles cs_cd.TextChanged   '160323smk 거래처,구분 색상변경
        ...
        If cs_gu.Text <> "CD0541" Then
            cs_cd.TextForeColor = Color.White
            cs_cd.TextBackColor = Color.Red

            cs_gu.TextForeColor = Color.White
            cs_gu.TextBackColor = Color.Red

        Else
            cs_cd.TextForeColor = Color.Black
            cs_cd.TextBackColor = Color.White

            cs_gu.TextForeColor = Color.Black
            cs_gu.TextBackColor = System.Drawing.Color.FromArgb(CType(CType(247, Byte), Integer), CType(CType(245, Byte), Integer), CType(CType(241, Byte), Integer))
    ...
    Private Sub set_tmp_TextChanged(sender As System.Object, e As System.EventArgs) Handles set_tmp.TextChanged
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1120_settmp", p)
    ...
    Private Sub g_list_Load(sender As Object, e As EventArgs) Handles g_list.Load

    End Sub


    'Private Sub g_body_AddedRow(sender As Object, RowIndex As Integer) Handles g_body.AddedRow
    '    If g_body.RowCount = 1 Then Exit Sub
    ...
    '    g_body.Text("AV_QTY", RowIndex) = g_body.Text("AV_QTY", RowIndex - 1)
    '    g_body.Text("MI_CHUL_QTY", RowIndex) = g_body.Text("MI_CHUL_QTY", RowIndex - 1)
    '    g_body.Text("OD_QTY", RowIndex) = ""
    '    g_body.Text("OD_PRI", RowIndex) = g_body.Text("OD_PRI", RowIndex - 1)
