...
Public Class WI_SA1150_PB
    ...
    Private Function Check_Stts() As String
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1150_stts_PB", p)
    ...
    Private Sub Max_NO()
        rt_no.CodeNo = "WI_SA1150_PB"        'Function dbo.[fnCodeNo]안에 LEB100에 대한 코딩을 해야 한다
    ...
    Private Function ConFrim() As Boolean
        ...
        Else
            ...
            If DIS_GU.Text = "1" Then       '20140702 반품이면,
                p.Add("@stts", "A")         '그대로 STTS = A
            Else                            ' 매출할인이면, 
                p.Add("@stts", "C")         'STTS = C
            End If
            'p.Add("@stts", "A")

            Me.Open("wi_sa1150_confirm_PB", p)

            p.Clear()
            p.Add("@rt_no", rt_no.Text)
            Me.Open("wi_sa1150_head_PB", p)
            p.Clear()
            p.Add("@rt_no", rt_no.Text)
            Me.Open("wi_sa1150_body_PB", p)
    ...
    Private Function DeConFrim() As Boolean
        ...
        ElseIf Check_Stts() = "C" Then
            ...
            If DIS_GU.Text = "2" Then
                'Return True
            Else
                MsgBox("반품 승인된 자료입니다.!")
                Return False
            End If
            'MsgBox("반품 승인된 자료입니다.!")  '20140703 원본 
            'Return False
        End If
    ...
    Private Sub Clear()
        ...
        cs_cd.Focus() 'smk 150807 취소후, 거래처커서위치
    End Sub
        ...
        Select Case ColumnName
            Case "HOT_CD"
                Try
                    ...
                    setPrice()   '151112smk 단축키 입력시, 자동단가등록
...
#Region "컬럼값 변경 Function"
    ...
    Private Sub setPrice()
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1150_price_PB", p)
        ...
        If IsEmpty(dSet) Then
            g_body.Text("RT_PRI", g_body.RowIndex) = ""
        ...
        For Each dRow In dRows
            g_body.Text("RT_PRI", g_body.RowIndex) = ToStr(dRow("price"))
    ...
    Private Sub popUP(hot_cd As String)
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1150_hot_cd_PB", p)
    ...
    Private Sub spec(hot_cd As String)
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1150_spec_PB", p)
        'Dim nm_cd As String = "", nm_nm As String = ""
        Dim dRow As DataRow, dRows As DataRowCollection
    ...
    Private Sub hot_spec(hot_cd As String)
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1150_hot_spec_PB", p)
        'Dim nm_cd As String = "", nm_nm As String = ""
        Dim dRow As DataRow, dRows As DataRowCollection
    ...
    'ENTER로 구분에서 단축키로 이동 '20151208 smk CS이주희요청
    'Private Sub g_body_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles g_body.KeyDown
    '    'If e.Control And e.KeyCode = Keys.Down Then
    '    '    Me.Mng_Focus()
    '    '    e.Handled = True
    '    'End If

    '    If g_body.FocusedFieldName = "GD_GU1" And e.KeyCode = Keys.Enter Then '구분
    '        g_body.FocusedFieldName = "HOT_CD"

    '        If g_body.Text("HOT_CD", g_body.RowIndex) <> "" Then '단축키 HOT_CD 
    '            SendKeys.Send("{ENTER}")

    '            If g_body.FocusedFieldName = "HOT_CD" And e.KeyCode = Keys.Enter Then

    '                g_body.FocusedFieldName = "SPEC30" 'POWER 
    '            End If
...
#End Region
    ...
    Private Function check_rm_qty() As Integer
        ...
        If ls_odgu = "CD05203" Then
            For i As Integer = 0 To g_body.RowCount - 1
                ...
                Me.Open("wi_sa1150_rmqty_chk_PB", p)

                Dim dSet As DataSet = Me.WorkSet("wi_sa1150_rmqty_chk_PB").DataSet
    ...
    Private Sub set_tmp_TextChanged(sender As System.Object, e As System.EventArgs) Handles set_tmp.TextChanged
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1150_settmp_PB", p)
        ...
        For Each dRow In dRows
            g_body.AddNewRow()
            g_body.Text("GD_GU1") = "1"
            g_body.Text("HOT_CD") = ToStr(dRow("hot_cd"))
            g_body.Text("GD_CD") = ToStr(dRow("gd_cd"))
            g_body.Text("RRT_QTY") = ToStr(dRow("qty"))

            'Me.setPrice()

            If ToStr(dRow("AMT")) <> "" Then
                g_body.Text("RT_PRI") = ToStr(dRow("AMT"))
            End If

        Next
    End Sub

    Private Sub cs_cd_TextChanged(sender As Object, oldValue As String) Handles cs_cd.TextChanged   '20150911smk 거래처입력시 자동행추가
