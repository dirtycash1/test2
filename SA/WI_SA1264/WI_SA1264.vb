...
Public Class WI_SA1264
    ...
    Private Function Check_Stts() As String
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("WI_SA1264_stts", p)
    ...
    Private Function Check_deconfirm() As String
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1264_deconfirm", p)
    ...
    Private Sub Max_NO()
        ' p/o 번호 채번
        job_no.CodeNo = "WI_SA1260"        'Function dbo.[fnCodeNo]안에 LEB100에 대한 코딩을 해야 한다
    ...
    Private Function ConFrim() As Boolean
        ...
        If MessageYesNo("확인 하시겠습니까?") = MsgBoxResult.No Then
            Exit Function
        Else
            'Me.Open("WI_SA1260_confirm")

            p.Clear()
            p.Add("@job_no", job_no.Text)
            p.Add("@stts", "C")

            Me.Open("WI_SA1264_confirm", p)
    ...
    Private Sub Clear()
        ...
        Me.Open("WI_SA1264_head", p)
        Me.Open("WI_SA1264_body", p)
    ...
    '    Dim dSet As Data.DataSet = Me.OpenDataSet("WI_SA1260_hot_cd", p)
    ...
    Private Sub s_btn_Click(sender As System.Object, e As System.EventArgs) Handles s_btn.Click
        Dim popup As New Form1

        'If g_body.ColumnIndex <> 0 Then Exit Sub

        'popup.InitPopup(g_body.Text("GD_CD"), g_body.Text("GD_NM"), g_body.Text("SEQ"), g_body.Text("OD_QTY"))
        popup.InitPopup(od_no.Text)

        If popup.ShowDialog() = DialogResult.OK Then
            If popup.ActiveControl.Tag IsNot Nothing Then
                Dim pReturn() As String = popup.ActiveControl.Tag
                popup.ActiveControl.Tag = Nothing
                '결과 파라미터를 받아서 처리할경우
                For i = 0 To pReturn.Length - 1
                    'g_multi.AddNewRow()

                    cs_cd.Text = pReturn(2)
                    cs_nm.Text = pReturn(3)
                    gd_cd.Text = pReturn(4)
                    gd_nm.Text = pReturn(5)
                    od_qty.Text = pReturn(6)
                    'gd_cd.Text = pReturn(7)
                    'gd_nm.Text = pReturn(8)
                    job_qty.Text = pReturn(9)
                    od_no.Text = pReturn(1)
                Next
    ...
    Private Sub g_body_CellValueChanged(sender As Object, ColumnName As String, RowIndex As Integer, ByRef Value As Object) Handles g_body.CellValueChanged
        Select Case ColumnName
            Case "JOB_QTY"
                p.Clear()
                p.Add("@gd_cd", g_body.Text("S_GDCD", RowIndex))

                Dim dSet As Data.DataSet = Me.OpenDataSet("WI_SA1264_qty", p)
                Dim l_qty As Decimal = 0

                If Not IsEmpty(dSet) Then l_qty = DataValue(dSet)

                g_body.Text("QTY", RowIndex) = l_qty * g_body.ToDec("JOB_QTY", RowIndex)
        End Select


    End Sub

    Private Function Check_QTY() As String
        p.Clear()
        p.Add("@gd_cd", job_no.Text)
