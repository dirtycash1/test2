...
Public Class WI_SA1265
    ...
    Private Function Saves() As Boolean
        ...
        If MessageYesNo("저장 하시겠습니까?") = MsgBoxResult.No Then
            Exit Function
        Else
            If Not Check_QTY() Then Return False
    ...
    Private Function Check_Stts() As String
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("WI_SA1265_stts", p)
    ...
    Private Function ConFrim() As Boolean
        ...
        If MessageYesNo("확인 하시겠습니까?") = MsgBoxResult.No Then
            Exit Function
        Else
            'Me.Open("WI_SA1265_confirm")

            p.Clear()
            p.Add("@pr_no", pr_no.Text)
            p.Add("@stts", "C")

            Me.Open("WI_SA1265_confirm", p)
    ...
    Private Function Check_deconfirm() As String
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_sa1265_deconfirm", p)
    ...
    Private Sub Clear()
        ...
        Me.Open("WI_SA1265_head", p)
        Me.Open("WI_SA1265_body", p)
        Me.Open("WI_SA1265_body1", p)

        p.Add("@pr_seq", 0)
        Me.Open("WI_SA1265_body2", p)
    ...
    Private Function Check_QTY() As Boolean
        ...
        With g_body1
            For i As Integer = 0 To .RowCount - 1
                If (g_body1.ToDec("QTY", i) = 0) Then

                    MsgBox("부자재 및 포장재 수량을 입력 하세요")
    ...
    Private Sub g_list_DoubleClick1(sender As Object, e As System.EventArgs) Handles g_list.DoubleClick
        ...
        p.Clear()
        p.Add("@pr_no", g_list.Text("PR_NO", g_list.RowIndex))
        p.Add("@pr_seq", g_body.Text("PR_SEQ", g_body.RowIndex))
    ...
    Private Sub popUP(s As String)
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("WI_SA1265_hot_cd", p)
    ...
    Private Sub setJobNO()
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("WI_SA1265_jobno", p)
        ...
        If Not IsEmpty(dSet) Then
            ...
            For Each dRow In dRows
                ...
                job_qty.Text = ToStr(dRow("JOB_QTY"))
                pack_qty.Text = ToStr(dRow("PACK_QTY"))
                ...
                deli_date.Text = ToStr(dRow("DELI_DATE"))
            Next
        End If
        ...
        dSet = Me.OpenDataSet("WI_SA1265_bom", p)
        ...
        dSet = Me.OpenDataSet("WI_SA1265_set", p)
        Dim ll_row1 As Integer = 0
        ll_row1 = 0
        ...
        If Not IsEmpty(dSet) Then
            ...
            With g_body1
                ll_row1 = .RowIndex
                For Each dRow In dRows
                    .InsertNewRow(ll_row1)
                    .Text("JOB_NO", ll_row1) = ToStr(dRow("JOB_NO"))
                    .Text("JOB_SEQ", ll_row1) = ToStr(dRow("JOB_SEQ"))
                    .Text("PR_SEQ", ll_row1) = ToStr(dRow("JOB_SEQ"))
                    .Text("GD_CD", ll_row1) = ToStr(dRow("GD_CD"))
                    .Text("GD_NM", ll_row1) = ToStr(dRow("GD_NM"))
                    .Text("SPEC", ll_row1) = ToStr(dRow("SPEC"))
                    .Text("JOB_QTY", ll_row1) = ToStr(dRow("QTY")) ' * job_qty.ToDec)
                    .Text("G_QTY", ll_row1) = ToStr(dRow("PR_QTY"))
    ...
    Private Sub g_body2_BeforeMoveRow(sender As Object, RowIndex As Integer, ByRef AllowMove As Boolean) Handles g_body2.BeforeMoveRow
    ...
    'smk
    Private Sub g_body_Click(sender As Object, e As System.EventArgs) Handles g_body.Click
        'smk
        p.Clear()
        p.Add("@pr_no", g_list.Text("PR_NO", g_list.RowIndex))
        p.Add("@pr_seq", g_body.Text("PR_SEQ", g_body.RowIndex))
