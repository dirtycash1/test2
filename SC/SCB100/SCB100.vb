...
Public Class SCB100
    ...
    Public Sub Init_Form()
        doc_no.CodeNo = "SCB100"
        EPanel2.Dock = DockStyle.Fill
        EPanel4.Dock = DockStyle.Fill
    ...
    Public Overrides Sub MenuButton_Click(ByVal mty As MenuType)
        ...
        Select Case mty
            ...
            Case MenuType.Save
                ...
                If doc_no.Text <> "" And Parameter.Login.RegId.ToString <> cid.Text And Parameter.Login.UserKd = "SC700300" Then Exit Sub '본인이 아닌경우 수정 불가
                Save_Form()
    ...
    Private Function Save_Form3() As Boolean
        ...
        Try
            ...
            If Parameter.Login.RegId.ToString <> cid.Text Then
                Dim dSet As DataSet = Me.OpenDataSet("scb100_f30")    'test_workset이 WorkSet에 등록되어있어야 한다
    ...
    'FileDownLoad정보저장
    Private Function Save_Form4(GetSq As String) As Boolean

        '본인이 아닌 경우만 사용
        Try

            If Parameter.Login.RegId.ToString <> cid.Text Then
                Dim dSet As DataSet = Me.OpenDataSet("scb100_f40")
                If dSet Is Nothing Then
                    Return True
                End If
    ...
    Private Function DocDelete() As Boolean
        ...
        If Delete("scb100_f10", False) = ExcuteResult.Succeeded Then Return True
    End Function
        ...
        Try
            For Row As Integer = 0 To g20.RowCount - 1
                ...
                If g20.IsDataRow(0) Then
                    If FileDelete(g20.Text("file_id"), g20.Text("file_nm"), False) Then
    ...
    Private Sub g10_Click(sender As Object, e As System.EventArgs) Handles g10.Click
        ...
        If doc_no.Text <> "" Then Save("scb100_f10")
    ...
    Private Sub btn_secu_Click(sender As System.Object, e As System.EventArgs) Handles btn_secu.Click
        Save_Form()
        If doc_no.Text = "" Then Exit Sub
        g30.ShowFilterRow = True
        EPanel4.BringToFront()
    End Sub
    ...
    Private Sub EPanel4_SizeChanged(sender As Object, e As System.EventArgs) Handles EPanel4.SizeChanged
        'Dim Wid As Long = (EPanel4.Width * 0.5) - 2
        'If Wid < 0 Then Wid = 0
        ''btn_add.Left = Wid - btn_add.Width
        ''btn_del.Left = Wid + 4
        'SplitContainer3.SplitterDistance = Wid
    End Sub

    Private Sub EPanel1_SizeChanged(sender As Object, e As System.EventArgs) Handles EPanel1.SizeChanged
        'EPanelChg()
    ...
    Private Sub FIleView(GetFileID As String, GetFileNm As String, GetSq As String, Optional Path As String = Nothing, Optional SHW As Boolean = True)
        Try
            [Shared].FileDownLoad(GetFileID, GetFileNm, , SHW, , Path)
            Save_Form4(GetSq)
    ...
    Private Function FileDelete(GetFileID As String, GetFileNm As String, Optional YesNo As Boolean = True) As Boolean
        If GetFileID = "" Then Return True
        Try
            If [Shared].FileDelete(GetFileID, GetFileNm, , , YesNo) Then
                If YesNo Then
                    g20.AllowDeleteRows = True
        ...
        If dialog.ShowDialog() = DialogResult.OK Then
            Try
                ...
                For Each FilePath As String In dialog.FileNames
                    RtnFileID = ""
                    RtnFileID = [Shared].FileUpload("", FilePath, , False)
                    DualChk(RtnFileID, System.IO.Path.GetFileName(FilePath))
                Next
    ...
    Private Sub DualChk(GetFileID As String, GetFileNm As String)
        If GetFileID = "" Then Exit Sub
        With g20
            .Find("file_id=" & GetFileID)
            If .Text("file_id") <> GetFileID Then

                .AddNewRow()
                .Text("file_id") = GetFileID
    ...
    Private Sub g20_DoubleClick(sender As Object, e As System.EventArgs) Handles g20.DoubleClick
        ...
        FIleView(g20.Text("file_id"), g20.Text("file_nm"), g20.Text("sq"))
    End Sub
    ...
    Private Sub g20_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles g20.KeyDown
        ...
        If e.KeyCode = Keys.Delete Then
            FileDelete(g20.Text("file_id"), g20.Text("file_nm"))
        End If
    End Sub
    ...
    Private Sub btn_dn_Click(sender As System.Object, e As System.EventArgs) Handles btn_dn.Click
        ...
        For Row As Integer = 0 To g20.RowCount - 1
            If g20.IsDataRow(Row) Then
                FIleView(g20.Text("file_id", Row), g20.Text("file_nm", Row), g20.Text("sq", Row), Path, False)
            End If
        Next
    End Sub
    ...
    Private Sub g20_DragDrop(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles g20.DragDrop
        ...
        If e.Data.GetDataPresent(DataFormats.FileDrop.ToString) Then
            ...
            For Each fullPath As String In e.Data.GetData(DataFormats.FileDrop)
                Dim FileID As String = [Shared].FileUpload("", fullPath, , False) '[Shared].FileUpload("", fileNm)
                If FileID <> "" Then
                    Dim f As New System.IO.FileInfo(fullPath)
                    g20.AddNewRow()
                    g20.Text("file_id") = FileID
                    g20.Text("file_nm") = System.IO.Path.GetFileName(fullPath)
                End If
