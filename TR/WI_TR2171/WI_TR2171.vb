...
Public Class WI_TR2171
    ...
    Private Function Saves() As Boolean
        ...
        Else
            ...
            If MyBase.Save() Then
                ...
                Me.Open("wi_tr2171_insert_extr2172", p)
    ...
    Private Function Check_Stts() As String
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_tr2171_stts", p)
    ...
    Private Function Del() As Boolean
        ...
        Else
            ...
            Me.Open("wi_tr2171_delete", p)

            '화면 다시 조회
            p.Clear()
            p.Add("@IP_NO", "XXX")

            Me.Open("wi_tr2171_head", p)

            p.Clear()
            p.Add("@IP_NO", "XXX")

            Me.Open("wi_tr2171_body", p)

            p.Clear()
            p.Add("@IP_NO", "XXX")

            Me.Open("wi_tr2171_body1", p)
            '--
    ...
    Private Function ConFrim() As Boolean
        ...
        Else
            ...
            Me.Open("wi_tr2171_insert_actr1000", p)
    ...
    Private Sub wf_setitem(a_cscd As String, a_curcd As String)
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_tr2171_odno_ip", p)
        ...
        If Not IsEmpty(dSet) Then
            With g_body1
                ...
                For Each dRow In dRows
                    ...
                    .Text("BL_DT", iRow) = ToStr(dRow("BL_DT"))
                    .Text("GU", iRow) = ToStr(dRow("GU"))
    ...
    Private Function Setitem_Rate(ByVal c As String) As String
        p.Clear()
        p.Add("@CUR_CD", c)
        p.Add("@IP_DT", ip_dt.Text)

        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_tr2171_set_rate", p)
    ...
    Private Sub g_body_CellValueChanged(sender As Object, ColumnName As String, RowIndex As Integer, ByRef Value As Object) Handles g_body.CellValueChanged
        If ColumnName = "IP_AMT" Then
            ...
            Me.IPAMT()
            'Me.Set_Ipamt()
            'ElseIf ColumnName = "CUR_CD" Then
            '    Me.Setitem_Rate()
        ElseIf ColumnName = "CUR_CD" Then
            g_body.Text("RATE", RowIndex) = Me.Setitem_Rate(g_body.Text("CUR_CD", RowIndex))
            g_body.Text("WON_AMT", RowIndex) = Math.Round(g_body.ToDec("RATE", RowIndex) * g_body.ToDec("IP_AMT", RowIndex), 0)

            'wf_rate()
        ElseIf ColumnName = "IP_AMT" Then
            g_body.Text("WON_AMT", RowIndex) = Math.Round(g_body.ToDec("RATE", RowIndex) * g_body.ToDec("IP_AMT", RowIndex), 0)
        End If
    ...
    Private Function wf_check() As Integer
        ...
        If ld_sumip <> ld_sumip1 Then
            MsgBox("입금내역 합과 수주별수금내역 합이 같지 않습니다.")
            Return 1
        End If
    ...
    Private Function check_misu_amt() As Integer
        Dim ls_invno, ls_odno, ls_ipno, ls_gu As String
        ...
        For i As Integer = 0 To g_body1.RowCount - 1
            ...
            Me.Open("wi_tr2171_misu_chk1", p)

            Dim dSet As DataSet = Me.WorkSet("wi_tr2171_misu_chk1").DataSet
    ...
    Private Function wf_de_check() As Integer
        ...
        Dim dSet As Data.DataSet = Me.OpenDataSet("wi_tr2171_de_chk", p)
    ...
    Private Sub WF_Setitem_dwbody()
        Dim ls_curcd As String = ""

        If g_body.RowCount() > 0 Then
            g_body.RowIndex = 0

            For b As Integer = 0 To g_body.RowCount + 1
                ls_curcd = g_body.Text("CUR_CD", b)

                If ls_curcd = "" Then
                Else
                    g_body.Text("RATE", b) = Me.Setitem_Rate(g_body.Text("CUR_CD", b))
                    g_body.Text("WON_AMT", b) = Math.Round(g_body.ToDec("RATE", b) * g_body.ToDec("IP_AMT", b), 0)
                End If
            Next
        End If
    End Sub
